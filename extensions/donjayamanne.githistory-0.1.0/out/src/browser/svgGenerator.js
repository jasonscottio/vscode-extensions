...
(function () {
    ...
    */
    ...
    window.GITHISTORY.generateSVG = function () {
        ...
        var items = JSON.parse(document.querySelectorAll('div.json.entries')[0].innerHTML);
            ...
            for (j = 0; j < entry.parents.length; ++j) {
                ...
                if (j !== 0 || branches.length === 0) {
                    ...
                    if (fictionalBranches.length === 0 || !fictionalBranchesUsed) {
                        ...
                        var newOrigX = (index + 1 + 1) * xOffset;
                        var newX = (index + j + 1 + 1) * xOffset;
                        // Generate at least 10 fictional branches, so we can lay them out neatly
                        for (var counter = 1; counter < 11; counter++) {
                            var newOrigX_1 = (index + 1 + counter) * xOffset;
                            var newX_1 = (index + j + 1 + counter) * xOffset;
                            var fictionalBranch = 'M ' + newOrigX_1 + ' ' + currentY + ' L ' + newOrigX_1 + ' ' + topMostY + ' L ' + newOrigX_1 + ' ';
                            fictionalBranches.push({ path: fictionalBranch, x: newOrigX_1 });
                        }
                    }
                    branchFound = true;
